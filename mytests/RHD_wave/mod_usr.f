...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    ...
    press(ixImin1:ixImax1) = p0 !+ A_p*dsin(wavenumber*x(ixImin1:ixImax1,1))
  ...
  subroutine Initialize_Wave(level,qt,ixImin1,ixImax1,ixOmin1,ixOmax1,w,x)
    use mod_global_parameters
    use mod_fld
  ...
  subroutine specialvar_output(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x,normconv)
    ...
    double precision :: sol1(ixOmin1:ixOmax1), sol2(ixOmin1:ixOmax1),&
        sol3(ixOmin1:ixOmax1)

    w(ixOmin1:ixOmax1,nw+1) = 1.d0/A_rho*(w(ixOmin1:ixOmax1,rho_)/rho0 - 1.d0)
    w(ixOmin1:ixOmax1,nw+2) = 1.d0/A_e*((w(ixOmin1:ixOmax1,&
       e_)-half*w(ixOmin1:ixOmax1,mom(1))**2/w(ixOmin1:ixOmax1,&
       rho_))/eg0 - 1.d0)
    w(ixOmin1:ixOmax1,nw+3) = 1.d0/A_er*(w(ixOmin1:ixOmax1,r_e)/Er0 - 1.d0)
    w(ixOmin1:ixOmax1,nw+4) = 1.d0/A_v*w(ixOmin1:ixOmax1,&
       mom(1))/w(ixOmin1:ixOmax1,rho_)

    sol1(ixOmin1:ixOmax1) = dsin(wavenumber*x(ixOmin1:ixOmax1,&
       1)-omega*global_time)
    sol2(ixOmin1:ixOmax1) = dsin(wavenumber*x(ixOmin1:ixOmax1,&
       1)-omega*global_time)
    sol3(ixOmin1:ixOmax1) = dsin(wavenumber*x(ixOmin1:ixOmax1,&
       1)-omega*global_time)
    where (x(ixOmin1:ixOmax1,1) .gt. one)
      sol1(ixOmin1:ixOmax1) = sol1(ixOmin1:ixOmax1)*dexp(-(x(ixOmin1:ixOmax1,&
         1)-one)/L_thin)
      sol2(ixOmin1:ixOmax1) = sol2(ixOmin1:ixOmax1)*dexp(-(x(ixOmin1:ixOmax1,&
         1)-one)/L_thick)
      sol3(ixOmin1:ixOmax1) = sol3(ixOmin1:ixOmax1)*dexp(-(x(ixOmin1:ixOmax1,&
         1)-one)/L_3)
    endwhere
    w(ixOmin1:ixOmax1,nw+5) = sol1(ixOmin1:ixOmax1)
    w(ixOmin1:ixOmax1,nw+6) = sol2(ixOmin1:ixOmax1)
    w(ixOmin1:ixOmax1,nw+7) = sol3(ixOmin1:ixOmax1)
