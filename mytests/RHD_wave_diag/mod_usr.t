...
contains
  ...
  subroutine usr_init()
    ...
    {^IFONED call mpistop('test not possible in 1D')}
...
113    close(unitpar)
  ...
  subroutine Initialize_Wave(level,qt,ixI^L,ixO^L,w,x)
    ...
    double precision :: press(ixI^S), temp(ixI^S), r(ixI^S)

    r(ixI^S) = sum(x(ixI^S,:), dim=ndim+1)
...
{^IFTHREED      w(ixI^S, mom(3)) = ndim**(-0.5d0)*w(ixI^S, rho_)*A_v*dsin(wavenumber*r(ixI^S)-omega*global_time)}
      w(ixI^S, e_) = eg0 + A_e*dsin(wavenumber*r(ixI^S)-omega*global_time)

      press(ixI^S) = p0 + A_p*dsin(wavenumber*r(ixI^S)-omega*global_time)
      temp(ixI^S) = (const_mp*fld_mu/const_kb)*press(ixI^S)/w(ixI^S, rho_)&
      *(unit_pressure/unit_density)/unit_temperature
  ...
  subroutine boundary_conditions(qt,ixI^L,ixB^L,iB,w,x)
    ...
    double precision :: press(ixB^S), temp(ixB^S)
    integer :: ix^D
    ...
    w(ixB^S, e_) = eg0
    press(ixB^S) = p0
    temp(ixB^S) = (const_mp*fld_mu/const_kb)*press(ixB^S)/w(ixB^S, rho_)&
    *(unit_pressure/unit_density)/unit_temperature
