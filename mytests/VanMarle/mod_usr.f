...
module mod_usr
  ...
  double precision :: rho_bound, Gamma, T_bound, E_dot, L_bound, M_dot, R_star,&
      M_star, v_inf, start_Edot
...
contains
  ...
  subroutine params_read(files)
    ...
    namelist /wind_list/ rho_bound, Gamma, T_bound, E_dot, L_bound, M_dot,&
        R_star, M_star, v_inf, start_Edot
...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixImin1,ixImax1, ixOmin1,ixOmax1, w, x)
    ...
    double precision :: rr(ixImin1:ixImax1), bb
    ...
    where(x(ixImin1:ixImax1,1) .gt. 1.d0)
      vel(ixImin1:ixImax1) =  v_inf*(1.d0 - 0.99d0*(1.d0/x(ixImin1:ixImax1,&
         1)))**0.5d0
      w(ixImin1:ixImax1,rho_) = M_dot/(4.d0*dpi*vel(ixImin1:ixImax1)*x(&
         ixImin1:ixImax1,1)**2)
    ...
    w(ixOmin1:ixOmax1,i_diff_mg) = (const_c/unit_velocity)*lambda(&
       ixOmin1:ixOmax1)/(fld_kappa0*w(ixOmin1:ixOmax1,rho_))
  ...
  subroutine boundary_conditions(qt,ixImin1,ixImax1,ixBmin1,ixBmax1,iB,w,x)
    ...
    case(1)
      ...
      Local_gradE(ixImin1:ixImax1) = -F_bound*3*fld_kappa0*w(ixImin1:ixImax1,&
    ...
    case(2)
      Local_tauout(ixBmin1:ixBmax1) = fld_kappa0*w(ixBmin1:ixBmax1,&
  ...
  subroutine PseudoPlanar(qdt,ixImin1,ixImax1,ixOmin1,ixOmax1,iwmin,iwmax,qtC,&
    ...
    if (qtC .gt. start_Edot) then
      where (x(ixImin1:ixImax1,1) .lt. 1.d0)
        w(ixOmin1:ixOmax1,r_e) = w(ixOmin1:ixOmax1,r_e) + qdt*E_dot
      end where
    endif
  ...
  subroutine PseudoPlanarSource(ixImin1,ixImax1,ixOmin1,ixOmax1,w,x,source,&
    ...
    if (rhd_radiation_diffusion) then
      if (boundary) then
        rad_flux(ixOmin1:ixOmax1,rdir) = L_bound/(4.d0*dpi*x(ixOmin1:ixOmax1,&
           rdir)**2)
      else
