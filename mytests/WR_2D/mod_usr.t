...
113    close(unitpar)
  ...
  subroutine initial_conditions(ixI^L, ixO^L, w, x)
    ...
    where(x(ixI^S,1) .gt. 1.d0)
      vel(ixI^S) =  v_inf*(1 - 0.999d0*( 1.d0/x(ixI^S,1) + (1 - 1.d0/xprobmin1)))**0.5d0
  ...
  subroutine boundary_conditions(qt,ixI^L,ixB^L,iB,w,x)
    ...
    case(1)
      ...
      do ix1 = ixBmax1,ixBmin1,-1
        w(ix2,ixBmin2:ixBmax2,mom(1)) = w(ix2+1,ixBmin2:ixBmax2,mom(1)) &
        *(x(ix2+1,ixBmin2:ixBmax2,1)/x(ix2,ixBmin2:ixBmax2,1))**2
      enddo

      !> Get OPAL opacities by reading from table
      call phys_get_trad(w,x,ixI^L,ixI^L,Temp)
  ...
  subroutine OPAL_and_CAK(ixI^L,ixO^L,w,x,kappa)
    ...
    integer :: ix^D
    double precision :: Temp(ixI^S), vel(ixI^S), gradv(ixI^S)
    ...
    CAK(ixO^S) = kappa_e*cak_Q/(1-cak_a) &
  ...
  subroutine specialvar_output(ixI^L,ixO^L,w,x,normconv)
    ...
    w(ixO^S,nw+2) = Tgas(ixO^S)*unit_temperature
