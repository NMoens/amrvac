...
module mod_usr
  ...
  double precision, allocatable :: vr_sumt(:), rho_sumt(:), rho2_sumt(:), sumt(:)
  ...
  subroutine boundary_conditions(qt,ixI^L,ixB^L,iB,w,x)
    ...
    case(1)
      ...
      do ix1 = ixImin1,ixImax1
        Local_gradE(ix1,ixBmin2:ixBmax2) = -(F_bound-F_adv(ixBmax1,ixBmin2:ixBmax2))/w(nghostcells+1,ixBmin2:ixBmax2,i_diff_mg)
      enddo
...
!    endwhere
  ...
  subroutine collapse_to_1D()
    ...
    if (mype==0) then
      ...
      do i=1,domain_nx1
        write(unitanalysis,'(3f11.7,3i4)') rr(i), vr(i), vr_sumt(i)/sum_time, lvl_h(i), lvl_l(i)
      enddo
    close(unitanalysis)
    endif
