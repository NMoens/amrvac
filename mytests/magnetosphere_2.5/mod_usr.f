...
contains
  ...
  subroutine usr_params_read(files)
    ...
    namelist /star_list/ M_star, L_star, R_star, T_star, B_star, Omega_star,&
  ...
  subroutine initglobaldata_usr
    ...
    Omega_star = Omega_star*unit_time
    v_inf = (one-Gamma_e)**0.5d0*escape_speed*(alpha/(1-alpha))**0.5d0
    eta_confine = (B_star**2 / R_star**2)/(M_dot*v_inf)
  ...
  subroutine initial_conditions(ixImin1,ixImin2,ixImax1,ixImax2, ixOmin1,&
    ...
    w(ixImin1:ixImax1,ixImin2:ixImax2, mom(3)) = R_star*dsin(x(ixImin1:ixImax1,&
       ixImin2:ixImax2,2))*Omega_star

    w(ixImin1:ixImax1,ixImin2:ixImax2, mag(3)) = zero
  ...
  subroutine special_bound(qt,ixGmin1,ixGmin2,ixGmax1,ixGmax2,ixBmin1,ixBmin2,&
    ...
    case(1)
      ...
      w(ixBmin1:ixBmax1,ixBmin2:ixBmax2, mom(3)) = x(ixBmin1:ixBmax1,&
      ...
      do i=ixBmax1,ixBmin1,-1
        ...
        w(i,:,mag(3)) = w(i+1,:,mag(3))-(x(i+1,:,1)-x(i,:,1))*(w(i+2,:,&
           mag(3)) - w(i+1,:,mag(3)))/(x(i+2,:,1)-x(i+1,:,1))
      enddo
      ...
      do i = ixGmin2,ixGmax2
        do j = ixBmin1,ixBmax1
          ...
          w(j,i,mom(3)) = min(w(j,i,mom(3)),one)
          w(j,i,mom(1)) = max(w(j,i,mom(1)),-one)
          w(j,i,mom(2)) = max(w(j,i,mom(2)),-one)
          w(j,i,mom(3)) = max(w(j,i,mom(3)),-one)
        enddo
      enddo

    case(3)

      w(ixBmin1:ixBmax1,ixBmin2:ixBmax2,mom(3)) = zero
      w(ixBmin1:ixBmax1,ixBmin2:ixBmax2,mag(3)) = zero
      if(mhd_glm) w(ixBmin1:ixBmax1,ixBmin2:ixBmax2,psi_)=0.d0
