...
contains
  ...
  subroutine usr_params_read(files)
    ...
    namelist /star_list/ M_star, L_star, R_star, T_star, B_star, Omega_star, rho_bound, alpha, qbar, kappa_e, beta
  ...
  subroutine initial_conditions(ixI^L, ixO^L, w, x)
    ...
    w(ixI^S, mom(3)) = R_star*dsin(x(ixI^S,2))*Omega_star
  ...
  subroutine special_bound(qt,ixG^L,ixB^L,iB,w,x)
    ...
    case(1)
      ...
      w(ixB^S,mom(2)) = zero
      w(ixB^S, mom(3)) = x(ixB^S,1)*dsin(x(ixB^S,2))*Omega_star
      ...
      do i=ixBmax1,ixBmin1,-1
        ...
        *(w(i+2,:,mag(2)) - w(i+1,:,mag(2)))/(x(i+2,:,1)-x(i+1,:,1))
        w(i,:,mag(3)) = w(i+1,:,mag(3))-(x(i+1,:,1)-x(i,:,1))&
        *(w(i+2,:,mag(3)) - w(i+1,:,mag(3)))/(x(i+2,:,1)-x(i+1,:,1))
      enddo
