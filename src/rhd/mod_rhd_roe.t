...
contains
  ...
  subroutine geteigenjump2_iso(wL,wR,wroe,x,ix^L,il,idim,smalla,a,jump,csound)
    ...
    select case (typeaverage)
    case ('arithmetic')
       csound(ix^S)=sqrt(rhd_adiab*rhd_gamma*&
            wroe(ix^S,rho_)**(rhd_gamma-one))
    ...
    case ('roe','default')
       where(abs(wL(ix^S,rho_)-wR(ix^S,rho_))<=qsmall*(wL(ix^S,rho_)+wR(ix^S,rho_)))
          csound(ix^S)=sqrt(rhd_adiab*rhd_gamma*&
               wroe(ix^S,rho_)**(rhd_gamma-one))
       elsewhere
          csound(ix^S)=sqrt(rhd_adiab*(wR(ix^S,rho_)**rhd_gamma-&
               wL(ix^S,rho_)**rhd_gamma)/(wR(ix^S,rho_)-wL(ix^S,rho_)))
       end where
    ...
    case('harten','powell')
       ...
       if (il == soundRW_) then
          tmp(ix^S) =wL(ix^S,mom(idim))/wL(ix^S,rho_)&
               + sqrt(rhd_adiab*rhd_gamma*wL(ix^S,rho_)**(rhd_gamma-one))
          tmp2(ix^S)=wR(ix^S,mom(idim))/wR(ix^S,rho_)&
               + sqrt(rhd_adiab*rhd_gamma*wR(ix^S,rho_)**(rhd_gamma-one))
       else if (il == soundLW_) then
          tmp(ix^S) =wL(ix^S,mom(idim))/wL(ix^S,rho_)&
               - sqrt(rhd_adiab*rhd_gamma*wL(ix^S,rho_)**(rhd_gamma-one))
          tmp2(ix^S)=wR(ix^S,mom(idim))/wR(ix^S,rho_)&
